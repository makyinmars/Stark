import type { GetServerSidePropsContext, NextPage } from "next";
import Image from "next/image";
import Head from "next/head";
import { useSession, signIn, signOut } from "next-auth/react";

import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "src/components/ui/dialog";
import { Button } from "src/components/ui/button";
import { ssgHelper } from "src/utils/ssg";
import { api } from "src/utils/api";
import Link from "next/link";

const Home: NextPage = () => {
  const utils = api.useContext();
  const { data: session } = useSession();
  const user = utils.auth.getUserSession.getData();

  return (
    <>
      <Head>
        <title>Stark</title>
        <meta name="description" content="Generated by create-t3-app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <main className="container flex flex-col gap-12 p-4 mx-auto">
        <nav className="flex items-center justify-between">
          <div className="text-2xl font-extrabold uppercase md:text-4xl">
            Stark
          </div>
          <Dialog>
            <DialogTrigger asChild>
              <Button>{session ? "Account" : "Get Started"}</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle>{session ? "Account" : "Sign In"}</DialogTitle>
                <DialogDescription>
                  {session ? "Time to Get Healthy" : "Join Stark with Discord"}
                </DialogDescription>
              </DialogHeader>
              {session ? (
                <div className="flex flex-col justify-center gap-4">
                  <h3 className="self-center">Signed in as: {user?.name}</h3>
                  <Link href="/dashboard">
                    <Button className="w-full">Dashboard</Button>
                  </Link>
                  <Button
                    onClick={() =>
                      void signOut({
                        callbackUrl: "/",
                      })
                    }
                  >
                    Sign Out
                  </Button>
                </div>
              ) : (
                <div className="flex flex-col justify-center gap-4">
                  <Button
                    onClick={() =>
                      void signIn("discord", {
                        callbackUrl: "/",
                      })
                    }
                  >
                    Discord
                  </Button>
                  <Button
                    onClick={() =>
                      void signIn("google", {
                        callbackUrl: "/",
                      })
                    }
                  >
                    Google
                  </Button>
                </div>
              )}
            </DialogContent>
          </Dialog>
        </nav>

        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <p className="text-sm md:text-base">
              Simple, Easy, Web Hosting - 10x Faster
            </p>
            <h1 className="text-lg font-semibold md:text-xl">
              Your Managed Cloud Hosting Platform
            </h1>
            <p>
              Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
              sint cillum sint consectetur cupidatat. Lorem ipsum dolor sit
              amet, qui minim labore adipisicing minim sint cillum sint
              consectetur cupidatat.
            </p>
            <Button className="font-bold">Start For Free</Button>
          </div>
          <Image
            src="/assets/exercise-1.webp"
            className="object-cover mx-auto"
            alt="Exercise 1"
            priority={true}
            width={300}
            height={300}
          />
        </div>

        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <p className="text-sm md:text-base">About Us</p>
            <h3 className="text-lg font-semibold md:text-xl">
              A Managed Cloud Hosting Platform Where Teams Can Build
            </h3>
            <p>
              Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
              sint cillum sint consectetur cupidatat. Lorem ipsum dolor sit
              amet, qui minim labore adipisicing minim sint cillum sint
              consectetur cupidatat.
            </p>
            <Button className="font-bold">Read More</Button>
          </div>

          <Image
            src="/assets/exercise-2.webp"
            className="object-cover mx-auto"
            alt="Exercise 2"
            priority={true}
            width={300}
            height={300}
          />
        </div>

        <div className="flex flex-col gap-4 p-4 rounded bg-slate-100">
          <h2>The Core Values We Live By</h2>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="flex flex-col ga-4">
              <h3>Innovative At Heart</h3>
              <p>
                Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
                sint cillum sint consectetur cupidatat.
              </p>
            </div>
            <div className="flex flex-col ga-4">
              <h3>Data Drive Ideology</h3>
              <p>
                Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
                sint cillum sint consectetur cupidatat.
              </p>
            </div>
            <div className="flex flex-col ga-4">
              <h3>Simpliciy in Everything</h3>
              <p>
                Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
                sint cillum sint consectetur cupidatat.
              </p>
            </div>
            <div className="flex flex-col ga-4">
              <h3>Win As A Team</h3>
              <p>
                Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
                sint cillum sint consectetur cupidatat.
              </p>
            </div>
          </div>
        </div>

        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <h3 className="text-lg font-semibold md:text-xl">
              Have Complete Peace Of Mind with Our Free Migration
            </h3>
            <p>
              Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
              sint cillum sint consectetur cupidatat. Lorem ipsum dolor sit
              amet, qui minim labore adipisicing minim sint cillum sint
              consectetur cupidatat.
            </p>
            <Button className="font-bold">Read More</Button>
          </div>

          <Image
            src="/assets/exercise-3.webp"
            className="object-cover mx-auto"
            alt="Exercise 3"
            priority={true}
            width={300}
            height={300}
          />
        </div>

        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <h3 className="text-lg font-semibold md:text-xl">
              Have Complete Peace Of Mind with Our Free Migration
            </h3>
            <p>
              Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
              sint cillum sint consectetur cupidatat. Lorem ipsum dolor sit
              amet, qui minim labore adipisicing minim sint cillum sint
              consectetur cupidatat.
            </p>
            <Button className="font-bold">Read More</Button>
          </div>

          <Image
            src="/assets/exercise-4.webp"
            className="object-cover mx-auto"
            alt="Exercise 4"
            priority={true}
            width={300}
            height={300}
          />
        </div>

        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2 p-4 rounded bg-slate-100">
            <h3 className="text-lg font-semibold md:text-xl">
              Have Complete Peace Of Mind with Our Free Migration
            </h3>
            <p>
              Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
              sint cillum sint consectetur cupidatat. Lorem ipsum dolor sit
              amet, qui minim labore adipisicing minim sint cillum sint
              consectetur cupidatat.
            </p>
            <Button className="font-bold">Read More</Button>
          </div>
          <div className="flex flex-col items-center justify-center gap-4 p-4">
            <h3>
              Stark changes my life, save me thousands of hours, and made
              working out a dream
            </h3>
            <p>
              Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
              sint cillum sint consectetur cupidatat.
            </p>
            <div className="flex items-center gap-4">
              <Image
                src="https://github.com/shadcn.png"
                alt="Avatar"
                className="rounded"
                priority={true}
                width={100}
                height={100}
              />
              <div className="flex flex-col gap-2">
                <h4>User Name</h4>
                <p>
                  Lorem ipsum dolor sit amet, qui minim labore adipisicing minim
                  sint cillum sint consectetur cupidatat.
                </p>
              </div>
            </div>
          </div>
        </div>
      </main>
      <footer className="flex items-center justify-center gap-4 mx-auto">
        <p>Product</p>
        <p>Quick Links</p>
        <p>Source</p>
        <p>Support</p>
      </footer>
    </>
  );
};

export default Home;

export const getServerSideProps = async (
  context: GetServerSidePropsContext
) => {
  const { ssg, session } = await ssgHelper(context);

  if (session && session.user) {
    await ssg.auth.getUserSession.prefetch();

    return {
      props: {
        trpcState: ssg.dehydrate(),
      },
    };
  } else {
    return {
      props: {
        trpcState: ssg.dehydrate(),
      },
    };
  }
};
